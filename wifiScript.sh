...
for node in $(seq 1 15);
 ...
 for iteration in $(seq 1);
  do
  ./waf --run "scratch/wifiTest --nWifi802_11b=$node --wifiMgr=arf" &>>output_file_arf.dat
  ./waf --run "scratch/wifiTest --nWifi802_11b=$node --wifiMgr=aarf" &>>output_file_aarf.dat
 done
done
