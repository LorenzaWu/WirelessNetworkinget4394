...
for iteration in $(seq 1 10);
 ...
 for node in $(seq 1 12);
  do
  ./waf --run "scratch/wifiRateAd --nWifi802_11b=$node --wifiMgr=arf" &>>output_file_arf.dat
  ./waf --run "scratch/wifiRateAd --nWifi802_11b=$node --wifiMgr=aarf" &>>output_file_aarf.dat
